  ...
  return (
    <>
      <nav className="container py-0 font-medium text-black uppercase xl:px-0">
        <ul className="flex items-center justify-center *:h-full *:hover:cursor-pointer *:hover:bg-black/10 xl:px-0">
          {links?.map((link, index) =>
            link.subCategory ? (
              <li key={`nav-item-${index}`} className="group relative">
                <Link
                  href={link.href}
                  className="inline-block h-full w-full px-4 py-3 lg:px-7 xl:px-8"
                >
                  {link.label}
                </Link>
                <ul className="invisible absolute top-full left-0 z-50 min-w-[200px] bg-white/95 opacity-0 shadow-lg backdrop-blur-3xl transition-all duration-200 ease-in-out group-hover:visible group-hover:opacity-100">
                  {link.subCategory.map((sub, subIndex) => (
                    <li key={`sub-nav-${subIndex}`}>
                      <Link
                        href={sub.href}
                        className={`hover:bg-accent hover:text-accent-foreground text-md inline-block h-full w-full px-4 py-2 transition-colors hover:bg-black/10 ${
              ...
              <li key={`nav-item-${index}`}>
                <Link
                  href={link.href}
                  className={`text-md inline-block h-full px-4 py-3 whitespace-nowrap lg:px-7 xl:px-8 ${
